...
class DLogoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double l = size.width * 0.15;
    final double r = size.width * 0.95;
    final double t = size.height * 0.05;
    final double b = size.height * 0.95;
    final double curveStart = size.width * 0.5;

    dPath.moveTo(l, t);
    dPath.lineTo(curveStart, t);
    dPath.cubicTo(r, t, r, b, curveStart, b);
    dPath.lineTo(l, b);
    dPath.close();

    // 2. Create the precise Lightning Bolt path
    ...
    final Path finalPath = Path.combine(PathOperation.difference, dPath, boltPath);
    canvas.drawPath(finalPath, paint);

    // 4. Neon accent glow at center zigzag
    final Paint accentPaint = Paint()
      ..shader = const LinearGradient(
        colors: [AppColors.neonPurple, AppColors.electricBlue],
